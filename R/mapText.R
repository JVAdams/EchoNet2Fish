...
#'   the same length as \code{lat} and \code{lon}.
#' @param cex
...
#'   pch=16, xla=0.4)
#'  mapText(lat=latitude, lon=longitude, group=basin)
#' }

mapText <- function(lat, lon, group, cex=1.5, ...) {
	# add text to plot at group lat/lon midpoints
	sug <- sort(unique(group))
	lng <- tapply(lon, group, midpoint)
