---
title: "EchoNet2Fish_Introduction"
author: "David Warner"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{EchoNet2Fish_Introduction}
...
For each set of data for which you would like to generate estimates (e.g., for one year of data from one lake), you should have a single sub directory (`subdir`) containing all of the relevant files.  For example, you might have a subdirectories called *MI2021* and *MI2022* containing all of the data for Lake Michigan in 2021 and 2022.  Within this subdirectory, you should have two more subdirectories for the acoustic data (one for all the Sv files and one for all the TS files), and you should have all of your midwater trawl files (operations, catches, lengths, and age-length keys).  Above this subdirectory should be an overarching directory (`refdir`) that contains a reference csv file (`refcsv`).
...
```{r refcsv, echo=FALSE}
library(DT)
refdat <- read.csv("C:/Users/dmwarner/Documents/Acoustics/EchoNet2Fish_LM/Reference.csv")
...
library(DT)
mydir <- readAll(refdir="C:/Users/dmwarner/Documents/Acoustics/EchoNet2Fish_LM/", keyvals=c(2, 2022),
...
datatable(look, rownames=F, caption = "Example Sv data")
```
...
look$Dummy_ID <- rep(1, 5)
datatable(look, rownames=F, caption = "Example target strength (TS) data")
```
...
datatable(look, rownames=F, caption = c("Example trawl operations data"))
```
...
datatable(look, rownames=FALSE, caption = "Example trawl catch data")
```
...
datatable(look, rownames=FALSE, caption = "Example length frequency (LF) data")
```
...
datatable(look, rownames=FALSE, caption = "Example alewife age-length key")
```
...
```{r, echo=FALSE}
mydir <- "C:/Users/dmwarner/Documents/Acoustics/EchoNet2Fish_LM/MI2022/"
```
...
myspInfo <- data.frame(
  sp = c(106, 109, 204),
  spname = c("alewife", "rainbow smelt", "bloater"),
  lcut = c(100, 90, 80),
...
Or, perhaps you want to define five slices, using the same premise as before, but now dividing the epilimnion into five geographic strata as is done for actual analyses by USGS.  You might write your slice definition as
...
# read in the data
mydir <- readAll(refdir="C:/Temp", keyvals=c(2, 2022))
...
estimateLake(maindir=mydir, ageSp=106, region=Mreg, regArea=MArea,
  soi=c(106, 109, 204), spInfo=myspInfo, sliceDef=myslice1, short=FALSE, descr="vignette")
```
