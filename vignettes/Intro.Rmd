...
Install the **EchoNet2Fish** package.  *If you don't already have devtools installed, following the instructions at [Readme](https://github.com/JVAdams/EchoNet2Fish/blob/master/README.md).*
...
Now you're ready to explore the data with the `exploreACMT()` function.  The `AC` and `MT` arguments are used to indicate if you want to explore the AC and MT data.  The `ageSp` argument gives the species codes for species that you want to apply age-length keys to; in this example `ageSp=106` for alewife.  The `short` argument is used to indicate if the surveyed area is wider than tall; in this example `short=FALSE` because Lake Michigan is not wider than tall.

```{r, eval=FALSE}
exploreACMT(maindir=mydir, rdat="ACMT", AC=TRUE, MT=TRUE, 
  ageSp=106, short=FALSE)
```
...
You will need a definition of the *strata* used to design the survey.  These are assumed to be non-overlapping two-dimensional regions that correspond to `Region_name` in the SV and TS files and `Transect` in the midwater trawl operation file.  You will also need to supply the surface areas (in ha) that correspond to each stratum.
...
You will need a definition of the *"slices"* by which you wish to summarize the abundance estimates.  These slices represent subpopulations (or domains) for which you want separate estimates.  They are **not the same** as *strata* which are used in the design of the survey and the estimation of the total population.
...
Now you're ready to use the `estimateACMT()` function to generate lake-wide estimates in both number (millions) and biomass (t).  The `region` and `regArea` arguments are used to indicate the regional strata and their corresponding areas (in ha).  The `TSrange` is the target strength range of interest, and `psi` is the transducer-specific two-way equivalent beam angle in steradians.  Species of interest are specified by `soi`, and each of these species should have size information in the data frame specified by `spInfo`.  And the slice definitions by which the estimates should be summarized are specified by the `sliceDef` argument.
 
```{r, eval=FALSE}
estimateACMT(maindir=mydir, rdat="ACMT", ageSp=106, region=Mreg, regArea=MArea,
  TSrange=c(-60, -30), psi=0.007997566, soi=c(106, 109, 129),
  spInfo=myspInfo, sliceDef=myslice1, short=FALSE)
```
...
In addition, 8 different data frames of estimates are saved as objects in an RData file and are written to csv files: 

* lake-wide totals `laketots_millions` and `laketots_t` (in millions and t)
* lake-wide means `lakemeans_nph` and `lakemeans_gph` (in numbers and g per ha)
* interval means `intmeans_nph` and `intmeans_gph` (in numbers and g per ha)
* interval and layer means `intlaymeans_nph` and `intlaymeans_gph` (in numbers and g per ha)

The data frames of lake-wide totals and lake-wide means have a row for each species group, a column for each slice, and additional columns for the total (of all slices) with the standard error and relative standard error.

The data frames of the interval means have a row for each region and interval, a column for each species group, and additional columns for region area, and the interval bottom depth, latitude and longitude.

The data frames of the interval and layer means have a row for each region, interval, and layer, a column for each species group, and many additional columns.

The rtf, RData, and csv files are all named using the lake and the year.
...
# generate estimates
estimateACMT(maindir=mydir, ageSp=106, region=Mreg, regArea=MArea,
  soi=c(106, 109, 129), spInfo=myspInfo, sliceDef=myslice1, short=FALSE)
```
